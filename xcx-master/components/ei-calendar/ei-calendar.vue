<template>
  <view style="position: fixed;width: 100%;bottom: 0;left: 0;" v-if="visibleSync" class="ei-calendar" :class="{ 'ei-calendar--drawer': drawer, 'is-active': active}">
    ...
    <view class="ei-calendar__container">
      ...
      <swiper class="ei-calendar__container__content" vertical :current="currentIndex" :duration="duration" @animationfinish="animationfinish" skip-hidden-item-layout circular>
        <swiper-item style="height: 100%;" class="ei-calendar__container__content__item" v-for="(item, itemIndex) in showMonthList" :key="itemIndex">
...
<script>
import EiCalendarItem from './ei-calendar-item.vue';
    ...
    getDayOption({ year, month, day, virtual, isToday }) {
      ...
      if (!virtual) {
        ...
        if (typeof this.disabledDate === 'function') {
			console.log(typeof this.disabledDate)
          isDisabled = this.disabledDate.call(this.$parent, date);
        } else {
			console.log(111111111111111,this.disabledDate)
