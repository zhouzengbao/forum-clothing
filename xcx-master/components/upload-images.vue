	...
	<view class="upload-content">
		<block v-for="(item, index) in imageList" :key="index">
			<view class="upload-item">
				<image class="upload-img" :src="item.filePath" mode="aspectFill" @click.self="previewImage(index)"></image>
				<image class="upload-del-btn" 
					@click.self="delImage(index)" 
					...
					mode="scaleToFill" style="transform: translateX(1px);z-index: 9999;">
				</image>
	...
	watch:{
		...
		value:{
			handler(newval, oldName) {				
				if(newval){
					newval.split('@').forEach(img=>{
						this.imageList.push({filePath:img,src:img})
					})
				}
			},
			immediate: true,
		...
		async uploadFiles(images){
			...
			if(uploadUrl){
				...
				if(images.length > 0 && this.rduLength > 0){
					...
					uni.hideLoading();
					this.$emit('imageListMsg',this.imageList)
				}
			}else{
			...
			return new Promise((resolve, reject)=> {
				...
				this.uploadTask = uni.uploadFile({
					...
					success(uploadFileResult){
						// console.log(uploadFileResult,'上传图片返回')
						...
						if(uploadFileRes.code == 1 && uploadFileRes.data){
							...
							uni.hideLoading();
							that.$util.msg(uploadFileRes.info);
							reject('接口返回错误');
						}
					}, 
					fail(error){
			...
			uni.showModal({
				...
				success: (confirmRes)=> {
					...
					if (confirmRes.confirm) {
						...
						// console.log(4)
						that.$emit('imageListMsg',that.imageList)
					} 
				}
			});
		},
